...
--------------------------------------------------------------------------
PROVENANCE
  Copied from grabette/task.py (develop @0cb3453) for the casquette fleet
  prototype (copy-first-then-share). Casquette is a PURE fleet peer:
  camera + BMI088 IMU only, writing raw_video.mp4 + imu_data.json.

  Deviations from the grabette original (keep in sync until extracted to a
  shared package):
    * EpisodeInfo.angle_sample_count and all angle-sensor references removed
      (casquette has no angle sensor).
    * OAK-D detection dropped: has_imu keys off imu_data.json only (no
      oakd_imu.json; the imu_sample_count fallback via meta["oakd"] is gone).
    * Data-dir default sourced from casquette.config.settings.data_dir
      instead of a hard-coded ~/grabette-data.

  The full public interface the fleet + command dispatcher rely on is
  preserved verbatim (method names and semantics): episode_id_for,
  Task/Session/Episode models, TaskManager with get_or_create_task,
  create_episode, register_episode, discard_pending_episode, report_tasks,
  revision, rename_task, delete_task_by_name, delete_episode,
  set_episode_members / set_episodes_members, active-task getters/setters,
  the ephemeral session lock, tasks.json persistence + legacy sessions.json
  migration, and the episode archive/download helpers.
--------------------------------------------------------------------------

Vocabulary (shared with the casquette fleet):
...
# Process-wide record store — a device has a single capture pipeline, so one
# TaskManager. Grabette keeps this in its tasks router; casquette's fleet
# prototype defers the local UI/router, so the singleton lives here and the
# fleet command dispatcher reaches it via get_task_manager().
