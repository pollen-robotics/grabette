# Copied VERBATIM from grabette/capture_scheduler.py (develop @0cb3453) for the
# casquette fleet prototype — no deviations (it depends only on the injected
# backend + record-store interfaces, stdlib only). Keep in sync until extracted
# to a shared package. See casquette/fleet/__init__.py.
"""Schedules a capture start at a future UTC instant, for synchronized
multi-device group recordings.

Each device waits out T0 on its own NTP-disciplined clock, so the round-trip
latency to discover/notify peers only delays when T0 is picked, not how tightly
the group starts once it fires.
...
class CaptureScheduler:
    ...
    async def _wait_and_start(self, backend, sm, episode_dir: Path, target_utc: datetime) -> None:
        try:
            ...
            # variable hardware bring-up doesn't sit between T0 and the first
            # frame — that variance is what makes two devices' recordings drift
            # apart. At T0, start_capture then only starts the recording clock.
