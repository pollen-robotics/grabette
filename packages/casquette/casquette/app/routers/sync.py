...
casquette has no button so this is API-driven there) presses start, the
device's `/api/sync/start` endpoint:

  1. Loads the peer list from env var (CASQUETTE_PEERS / GRABETTE_PEERS).
...
Peer config format:
    CASQUETTE_PEERS="dev1=http://host1:8001,dev2=http://host2:8000"
...
def _build_rig(peers: list[Peer]) -> list[dict]:
    ...
    fall back to hostname if CASQUETTE_DEVICE_ID isn't set.
    """
    import socket
