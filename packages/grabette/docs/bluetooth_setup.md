...
> **On Linux, pair once from a terminal first.** If the tool connects and drops straight away (the Pi logs `RequestAuthorization` then a disconnect ~200ms later, the browser says `GATT Server is disconnected`), your laptop refused the pairing: its desktop Bluetooth agent didn't confirm it. Pair by hand once and the tool works normally afterwards — the bond persists. The scan prints the address, so this needs nothing but Bluetooth range:
> ```
> bluetoothctl
> agent on
> default-agent
> scan on            # wait for: [NEW] Device 88:A2:9E:.. Grabette (hostname)
> scan off
> pair 88:A2:9E:..   # the address the scan just printed
> ```
