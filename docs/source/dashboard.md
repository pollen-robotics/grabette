...
- **Speaker** — the volume of the recording cues. **Play start / stop cues** plays the real pair — the ascending beep that marks a take going live, then the descending one that marks it stopping — so the level is judged on the sounds you will actually have to recognise in the field. It is refused while a recording is running, since anyone in earshot would hear the take start and stop when neither happened. The level is remembered on the device and survives a restart. A Grabette built without a speaker is a supported configuration, not a fault: the control is greyed out and says so, and a level set anyway is kept in case one is fitted later.
